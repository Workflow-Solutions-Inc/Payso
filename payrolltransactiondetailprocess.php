...
if($_GET["action"]=="update"){
	...
	if($id != ""){
	 $sqlupdate = "UPDATE payrolldetailsaccounts SET
		...
		//$ActRate = 0;
			...
			$sqlinsert = "call sp_computationPayrollPerworker('$paynum' ,'$dataareaid' ,'$id')";
			//mysqli_query($conn,$sqlinsert);
			//echo $sqlinsert."<br>".$conn->error;
			if(mysqli_query($conn,$sqlinsert))
			{
	...
	if($paynum != ""){
		...
		$sqlinsert = "call sp_computationPayroll('$paynum' ,'$dataareaid')";
			//mysqli_query($conn,$sqlinsert);
			//echo $sqlinsert."<br>".$conn->error;
			if(mysqli_query($conn,$sqlinsert))
			{
	...
	if($id != ""){
	 $sqlupdate = "UPDATE payrolldetails SET
				islastpay = '1',
				modifiedby = '$userlogin',
				modifieddatetime = now()
				WHERE payrollid = '$paynum'
