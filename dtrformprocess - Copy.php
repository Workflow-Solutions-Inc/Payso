...
include("dbconn.php");
function getaccountsHeader($payID,$PayPer,$PayCut,$dtID,$conPAR)
{	
	$conn = $conPAR;
	$dataareaid = $dtID;
	$varPayCut = $PayCut;
	...
	$DHquery = "SELECT prdtls.workerid,prdtls.payrollid,prdtls.linenum,dtrheader.daysworked,dtrheader.hoursworked,dtrheader.overtimehours,dtrheader.nightdifhours,
						...
						dtrheader.holiday,dtrheader.holidaynd,dtrheader.holidayot,prdtls.contractid,dtrheader.absent from dailytimerecordheader dtrheader 
			...
			while ($row3 = $result3->fetch_assoc())
				{
					...
					else
					{
						...
						if($varPayCut == 1)
						{
							$query6 = "call SP_getWorkerRateforDeductions ('$dataareaid','$wrkId','$paynum');
										";
						...
						else //if($acccode2 == 'SSS' || $acccode2 == 'PH')
						{
							...
							$query11 = "call PT_Contribution('$dataareaid','$resformula','$acccode2')";
...
header('location: dtrform.php');
	...
	header('location: dtrform.php');
	 ...
	 $sql = "UPDATE dailytimerecorddetail SET
				timein = concat(date_format(timein, '%Y-%m-%d'),' ',TIME_FORMAT('$timein', '%H:%i')),
				timeout = concat(date_format(timeout, '%Y-%m-%d'),' ',TIME_FORMAT('$timeout', '%H:%i'))
		...
		if(mysqli_query($conn,$sql))
		{
			...
			$sqlgenerate = "call generateDTRMGCperworker('$dataareaid','$startdate','$enddate','$payrollperiod','$wkid')";
	...
	$Brnresult = $conn->query($Brnquery);
		...
		{ 
			...
			$sqlupdateDTR = "UPDATE dailytimerecordheader dtr left join worker wk on dtr.workerid = wk.workerid 
				...
				getaccountsHeader($sequence,$period,$dataareaid,$cutoff,$conn);
