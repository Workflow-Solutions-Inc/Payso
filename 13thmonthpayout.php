...
<body>-->
	...
	<div id="leftpanel" class="leftpanel">
		...
		<ul class="extrabuttons">
			<div class="leftpanel-title"><b>Post</b></div>
			<li><button onClick="generatereport()"><span class="fas fa-edit"></span> Generate Report</button></li>
		</ul> 
				...
				<div id="tablearea1" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mainpanel-area">
					<div class="mainpanel-content">
						...
						<div class="mainpanel-title">
							<span class="fa fa-archive"></span> 13th Month Payout Details
						</div>
							...
							<table width="100%" style="border: 1px solid #d9d9d9;" id="datatbl" class="table mainpanel-table">
								<thead>
									<tr class="rowtitle">
										...
										<td style="width:20%;">Fromdate</td>
										<td style="width:20%;">Todate</td>
									...
									<tr class="rowsearch">
									  <td class="text-center"><span class="fas fa-search fa-xs"></span></td>
										...
										<?php
											$query = "SELECT distinct thmonthpayoutid FROM thmonthpayoutheader where dataareaid = '$dataareaid'";
									  ...
									  <datalist id="SearchLeavePayoutid">
										...
										?>
											<option value="<?php echo $row["13thmonthpayoutid"];?>"></option>
										<?php } ?>
										</datalist>
									  </td>
									   ...
									   <td><input list="SearchYear" class="search" disabled>
										<?php
											$query = "SELECT distinct year FROM thmonthpayoutheader where dataareaid = '$dataareaid'";
											$result = $conn->query($query);		
									  	?>
									  ...
									  <td><input list="period" class="search" disabled>
										<?php
											$query = "SELECT distinct fromdate FROM thmonthpayoutheader where dataareaid = '$dataareaid'";
											$result = $conn->query($query);		
									  	?>
									  <datalist id="period">
										<?php 
											while ($row = $result->fetch_assoc()) {
										?>
											<option value="<?php if ($row['period'] == 0) {
												echo "First Half";
											} else {
												echo "Second Half";
											}?>"></option>
									  ...
									  <td><input list="SearchStatus" class="search" disabled>
										<?php
											$query = "SELECT distinct status FROM thmonthpayoutheader where dataareaid = '$dataareaid'";
											$result = $conn->query($query);		
									  	?>
								...
								<tbody id="result">
									<?php
									$query = "SELECT thmonthpayoutid,year,fromdate,todate,case when status = 1 then 'Posted' else 'Created' end as statustxt,status from thmonthpayoutheader
									...
									{ 
										...
										<tr class="<?php echo $rowclass; ?>">
											...
											<td style="width:20%;"><?php echo $row['thmonthpayoutid'];?></td>
											<td style="width:20%;"><?php echo $row['year'];?></td>
											<td style="width:20%;"><?php echo $row['fromdate'];?></td>
											<td style="width:20%;"><?php echo $row['todate'];?></td>
											<td style="width:20%;"><?php echo $row['statustxt'];?></td>
											<td style="display:none;width:1%;"><?php echo $row['status'];?></td>
										</tr>
									<?php 
									$firstresult = $row["thmonthpayoutid"];
									}
	...
	<div class="modal-container">
		<div class="modal-content">
			<div class="modal-header">
				<div class="col-lg-6">13th Month Details</div>
				<div class="col-lg-6"><span class="fas fa-times modal-close"></span></div>
			</div>
			
			<div id="container" class="modal-content-container">
				<form name="myForm" accept-charset="utf-8" action="13thmonthpayoutprocess.php" method="get">
					<div class="row">
						
						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<label>Payout ID:</label>
							<div id="resultid">
								<input type="text" value="" placeholder="Leave Payout ID" name ="monthpayoutid" id="add-monthpayoutid" class="modal-textarea" required="required">
							</div>
...
<script src="js/ajax.js"></script>
	<script  type="text/javascript">
		...
		var locStatus;
  		$(document).ready(function(){
			$('#datatbl tbody tr').click(function(){
				...
				locStatus = $("#datatbl tr:eq("+ ($(this).index()+2) +") td:eq(6)").text();
		...
		CreateBtn.onclick = function() {
		    ...
		    $.ajax({
						type: 'GET',
						url: '13thmonthpayoutprocess.php',
						data:{action:action},
						...
						success: function(data){
							$('#resultid').html(data);
							$("#add-monthpayoutid").prop('readonly', true); 
				}
			});
		...
		UpdateBtn.onclick = function() {
			if(so != '') {
				if (locStatus != '1') {
				    ...
				    document.getElementById("add-monthpayoutid").value = so;
		...
		function Delete()
		{
			...
			{
				alert("13th Month Payout has been Posted.");
			}	
		}
		...
		function linedetails()
		{
			if(so != ''){
				...
				$.ajax({
					type: 'GET',
					url: '13thmonthpayoutprocess.php',
					data:{action:action, thpayoutid:so, locfromdate:locfromdate, loctodate:loctodate, locYear:locYear, locStatus:locStatus},
					success: function(data) {
						so='';
					    window.location.href='13thmonthpayoutdetail.php';
				    }
				});
			}
			else {
		...
		function generatereport(){
			//var di = document.getElementById("hide2").value;
			

			if(so != ''){
				var di =  "<?php echo $dataareaid; ?>";
