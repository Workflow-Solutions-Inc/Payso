...
if(isset($_GET["save"])) {
	...
	if($_GET["actmode"]=="userform"){
		...
		$query = "SELECT 
					...
					where lf.dataareaid = '$dataareaid' and (lf.workerid like '%$id%') and (wk.name like '%$name%') and (lf.voucher like '%$voucher%')
	...
	if($_GET["actmode"]=="userform"){
		...
		if($act == 1)
		{
			$query = "SELECT 
					...
					where lf.dataareaid = '$dataareaid' and lf.workerid = '$wkidPros'  and lf.balance != 0 
		...
		else
		{
			$query = "SELECT 
					...
					where lf.dataareaid = '$dataareaid' and lf.workerid = '$wkidPros' 
...
else if($_GET["action"]=="unloadSched"){
	 	
	unset($_SESSION['WKNumSchedID']);
	unset($_SESSION['WKNumSchedVouch']);
	unset($_SESSION['WKNumSchedType']);
	//unset($_SESSION['paynum']);
	header('location: loanschedform.php');
	
}

else if($_GET["action"]=="unload"){
	 	
	unset($_SESSION['WKNumLoan']);
	//unset($_SESSION['paynum']);
	header('location: loanfileform.php');
