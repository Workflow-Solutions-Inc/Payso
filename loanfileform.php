...
<body>-->
	...
	<div id="leftpanel" class="leftpanel">
		...
		<ul class="subbuttons">
			...
			<li style="display: none;"><button id="myAddBtn"><span class="fa fa-plus"></span> Create Record</button></li>
			...
			<li class="LoanFileMaintain" style="display: none;"><button onClick="SetLoan();"><span class="fa fa-plus"></span> Set Loan</button></li>
			<li class="LoanFileMaintain" style="display: none;"><button onClick="Sched();"><span class="fa fa-plus"></span> View Loan Schedule</button></li>
					...
					<div class="row">
						...
						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							...
							<select placeholder="Account" id="add-account" name="LFaccount" class="modal-textarea" style="width:100%;height: 28px;"  required="required">
								<option value="" selected="selected"></option>
								<?php
									$query = "SELECT distinct accountcode,name FROM accounts where dataareaid = '$dataareaid' ";
									$result = $conn->query($query);			
...
<script src="js/ajax.js"></script>
	<script  type="text/javascript">
		...
		$('#view-archive').change(function(){
			archivecb = 0;
		    if($(this).attr('checked'))
		    {
          		$(this).val('1');
          		//archivecb = $(this).val('1');
          		//document.getElementById("inchide").value =1;
		    }
		    else
		    {
		         $(this).val('0');
		         //archivecb = $(this).val('0');
		        // document.getElementById("inchide").value=0;
		    }
		    archivecb = $('#view-archive').val();
		    var action = "archive";
			var actionmode = "userform";
		    $.ajax({
						type: 'GET',
						url: 'loanfileformprocess.php',
						data:{action:action, actmode:actionmode, archivecb:archivecb},
		...
		function SetLoan()
		{
			...
			window.location.href='loanwizard.php';
