...
<body>-->
	...
	<div id="leftpanel" class="leftpanel">
		...
		<ul class="subbuttons">
			...
			<li class="LoanTypeMaintain" style="display: none;"><button id="myAddBtn"><span class="fa fa-plus"></span> Create Record</button></li>
			<li class="LoanTypeMaintain" style="display: none;"><button onClick="Delete();"><span class="fa fa-trash-alt"></span> Delete Record</button></li>
			<li class="LoanTypeMaintain" style="display: none;"><button id="myUpdateBtn"><span class="fa fa-edit"></span> Update Record</button></li>
				...
				<div id="tablearea1" class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mainpanel-area">
					<div class="mainpanel-content">
						...
						<div class="mainpanel-title">
							<span class="fa fa-archive"></span> Loan Type
						</div>
							...
							<table width="100%" style="border: 1px solid #d9d9d9;" id="datatbl" class="table table-striped mainpanel-table">
								<thead>
									<tr class="rowtitle">
										...
										<td style="width:50%;">Loan Type ID</td>
									...
									<tr class="rowsearch">
									  <td class="text-center"><span class="fas fa-search fa-xs"></span></td>
										...
										<td><input list="SearchLoan" class="search">
										<?php
											$query = "SELECT distinct loantypeid FROM loantype";
											$result = $conn->query($query);	
												
									  ?>
									  <datalist id="SearchLoan">
										
										<?php 
										
											while ($row = $result->fetch_assoc()) {
										?>
											<option value="<?php echo $row["loantypeid"];?>"></option>
									  ...
									  <td><input list="SearchName" class="search">
										<?php
											$query = "SELECT distinct description FROM loantype";
											$result = $conn->query($query);	
								...
								<tbody id="result">
									...
									{ 
										...
										$rowcnt2++;
											...
											$collection = $collection.','.$row['loantypeid'];
										?>
										<tr id="<?php echo $row['loantypeid'];?>" class="<?php echo $rowclass; ?>" tabindex="<?php echo $rowcnt2; ?>">
											<!--<td style="width:10px;"><input type='checkbox' name="chkbox" value="" id="myCheck"></td>-->
											<td style="width:20px;" class="text-center"><span class="fa fa-angle-right"></span></td>
											<td style="width:50%;"><?php echo $row['loantypeid'];?></td>
	...
	<div class="modal-container">
		<div class="modal-content">
			<div class="modal-header">
				<div class="col-lg-6">Loan Type</div>
				<div class="col-lg-6"><span class="fas fa-times modal-close"></span></div>
			</div>
			
			<div id="container" class="modal-content-container">
				<form name="myForm" accept-charset="utf-8" action="loantypeformprocess.php" method="get">
					<div class="row">

						<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
							<label>Loan Type ID:</label>
							<input type="text" value="" placeholder="Loan Type ID" name ="LoanId" id="add-loan" class="modal-textarea" required="required">
...
<script src="js/ajax.js"></script>
	 <script  type="text/javascript">
		...
		CreateBtn.onclick = function() {
		    ...
		    $("#add-loan").prop('readonly', false);
		    document.getElementById("add-loan").value = '';
		...
		UpdateBtn.onclick = function() {
			if(so != '') {
			    //modal.style.display = "block";
			    $("#myModal").stop().fadeTo(500,1);
			    $("#add-loan").prop('readonly', true);
				document.getElementById("add-loan").value = so;
		...
		function checkExistForm()
		{
			...
			//alert(myId.length);
			var n = myId.includes(document.getElementById("add-loan").value.toLowerCase());
			//alert(n);
			if(n == true){
				alert("Loan Type ID already Exist!");
				return false;
			}
			else
			{
		...
		$( ".search" ).on( "keydown", function(event) {
		  if(event.which == 13){
			...
			var data=[];
			 ...
			 $.ajax({
						type: 'GET',
						url: 'loantypeformprocess.php',
						data:{action:action, actmode:actionmode, LoanId:LoanId, name:name},
			...
			if(so != '') {
				if(confirm("Are you sure you want to remove this record?")) {
					$.ajax({	
							...
							url: 'loantypeformprocess.php',
							//data:'action=save&actmode=userform&userno='+UId.value+'&pass='+UPass.value+'&lname='+NM.value+'&darea='+DT.value,
							data:{action:action, actmode:actionmode, LoanId:so},
							beforeSend:function(){
