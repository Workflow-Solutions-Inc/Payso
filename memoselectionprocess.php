...
if($_GET["action"]=="save"){
	 ...
	 $query = "SELECT * FROM worker
		...
		while ($row = $result->fetch_assoc())
		{
			...
			$sql = "INSERT INTO memodetail(memoid,workerid,status,dataareaid)
			values ('$memoheadid','$userid','Receiver','$dataareaid')";
	...
	header('location: memoselection.php');
	
}
else if($_GET["action"]=="savecc"){
	 ...
	 $query = "SELECT * FROM worker
		...
		while ($row = $result->fetch_assoc())
		{
			...
			$sql = "INSERT INTO memodetail(memoid,workerid,status,dataareaid)
			values ('$memoheadid','$userid','CC','$dataareaid')";
			if(mysqli_query($conn,$sql))
			{
				echo $sql;
			}
